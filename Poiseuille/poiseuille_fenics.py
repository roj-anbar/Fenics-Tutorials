...
beta = 18          #pressure gradient between inlet and outlet
...
# Approach1: use C++ syntax
inflow  = 'near(x[0], 0)'
outflow = 'near(x[0], 1)'
walls   = 'near(x[1], 0) || near(x[1], 1)'


# Approach2: define python functions
...
bcp_inflow  = DirichletBC(Q, Constant(beta), inflow) #inlet pressure: calculated based on analytic solution
...
xdmf_file_u = XDMFFile("./results/velocity.xdmf")
xdmf_file_p = XDMFFile("./results/pressure.xdmf")
...
for n in range(num_steps):
    ...
    # Define exact solution using C++ expressions
    u_e = Expression(('0.5*beta*x[1]*(1.0 - x[1])', '0'), beta = beta, degree=2)
...
#vtkfile_u = File('./results/velocity.pvd')
#vtkfile_p = File('./results/pressure.pvd')
